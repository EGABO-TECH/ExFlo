...
import { PilotPhaseType, PilotPhase } from "../services/agents/state";
...
export class BookingController {
   ...
   */
  static async planTrip(prompt: string, sessionId: string = "default-session") {
    try {
        // 1. Invoke LangChain AI Pilot to process the user's prompt using the multi-phase StateGraph
        const stateResult = await pilotAgent.invoke(
            { messages: [new HumanMessage(prompt)] },
            { configurable: { thread_id: sessionId } }
        );

        const phase: PilotPhaseType = stateResult.phase || PilotPhase.DISCOVERY;
        const aiResponse = stateResult.uiState?.aiResponse || "I am processing your request...";
        
        const items: any[] = [];
        let total = 0;

        // If we are in the Scheduling phase or Finished phase, extract the orchestrated items from tool messages.
        if (phase === PilotPhase.SCHEDULING || phase === PilotPhase.FINISHED) {
            for (const msg of stateResult.messages) {
            ...
            // Fallback if no valid tools called during orchestration
            if (items.length === 0) {
                items.push(await this.simulateDuffelFlight(stateResult.destination || "Destination"));
                items.push(await this.simulateTripAdvisorHotel(stateResult.destination || "Destination"));
                items.push(await this.simulateLocalGuideTour(stateResult.destination || "Destination"));
                total = items.reduce((acc, curr) => acc + curr.price, 0);
            }
        }

        return { items, total, aiResponse, phase };
    } catch (error) {
        ...
        return { items, total, aiResponse: "I encountered an error connecting to the AI Oracle, but I've assembled a fallback itinerary for you!", phase: PilotPhase.FINISHED };
    }
  }
}
